...
- **Supabase PostgreSQL** is the primary database; **Supabase Storage** holds media
  objects.
- All data access from the backend goes through repositories (ADR-0003) using the
  Supabase client with parameterized queries — **raw SQL string interpolation is
  forbidden**.
